...
const getProjects = async (req, res = response) => {
  ...
  await userRepository.all().then((data) => {
    ...
    res.status(200)
      .json({msg: 'Projects found', user: data});
...
const getProject = async (req, res = response) => {
  ...
  await userRepository.findById(id).then((data) => {
    ...
    res.status(200)
      .json({msg: 'Project found', user: data});
...
const setProjects = async (req, res = response) => {
  ...
  await userRepository.create(req.body).then((data) => {
    ...
    res.status(201)
      .json({msg: 'Project successfully created', user: data});
...
const updateProject = async (req, res = response) => {
  ...
  await userRepository.UpdateById(id, req.body).then((data) => {
    ...
    res.status(200)
      .json({msg: 'Project updated', user: data});
...
const deleteProject = async (req, res = response) => {
  ...
  await userRepository.deleteById(id).then((data) => {
    ...
    res.status(200)
      .json({msg: 'Project deleted', user: data});
